...
async function safeDbUpdate(table:string,updates:Record<string,any>,col:string,val:string){try{await sb.from(table).update(updates).eq(col,val);}catch(e){console.log(`[v51] DB update error (${table}): ${(e as Error).message}`);}}
...
async function rC(n:string,k:string):Promise<{id:string|null;log:string;error?:string;_409Debug?:any}>{
  ...
  try{const r=await G('/companies',k,{search:n,take:'50'});await W(DL);if(r?.Data)for(const x of r.Data)if(x.Id&&ok(x.Id)&&x.Name&&sm(n,x.Name)){CC[c]=x.Id;await safeDbUpdate('marketing_companies',{crelate_id:x.Id},'company_name',n);return{id:x.Id,log:'s'};}}catch{}
  ...
  try{const r=await Po('/companies',k,{Name:n});await W(DL);
    if(r.ok){const id=xI(r.data);if(id){CC[c]=id;await safeDbUpdate('marketing_companies',{crelate_id:id},'company_name',n);return{id,log:'+'};}else return{id:null,log:'x:no-id',error:`POST OK but no ID. Body: ${(r.rawBody||'').substring(0,200)}`};}
      ...
      if(r.status===409){
        ...
        console.log(`[v51] 409 for company "${n}", starting detailed debug search...`);
        ...
        try{
          ...
          console.log(`[v51] 409 Search 1 - URL: ${fullUrl}`);
          ...
          console.log(`[v51] 409 Search 1 results: ${resultCount} companies, names: ${JSON.stringify(resultNames.map((r:any)=>r.name))}`);
          ...
          console.log(`[v51] 409 Search 1 error: ${(e as Error).message}`);
        }

        // Search 2: First 3 words
        const words=n.trim().split(/\s+/).slice(0,3).join(' ');
        if(words&&words.toLowerCase().trim()!==n.toLowerCase().trim()){
          try{
            ...
            console.log(`[v51] 409 Search 2 (3-word) - URL: ${fullUrl}`);
            ...
            debugInfo.searches.push(search2Debug);
            console.log(`[v51] 409 Search 2 results: ${resultCount} companies, names: ${JSON.stringify(resultNames.map((r:any)=>r.name))}`);
            ...
            if(parsed?.Data){
              for(const x of parsed.Data){
                ...
                if(xLower===nLower||sm(n,x.Name)){
                  CC[c]=x.Id;await safeDbUpdate('marketing_companies',{crelate_id:x.Id},'company_name',n);
                  debugInfo.outcome = 'MATCHED on search 2 (3-word)';
                  debugInfo.matchedName = x.Name;
                  debugInfo.matchedId = x.Id;
                  return{id:x.Id,log:'409+s3w',_409Debug:debugInfo};
                }
              }
            }
            debugInfo.search2MatchResult = 'NO MATCH FOUND';
          }catch(e){
            debugInfo.searches.push({searchNumber:2,error:(e as Error).message});
            console.log(`[v51] 409 Search 2 error: ${(e as Error).message}`);
          }
        } else {
          debugInfo.search2Skipped = `3-word query "${words}" same as full name, skipped`;
        }
        ...
        console.log(`[v51] 409-nf FINAL for "${n}": ${JSON.stringify(debugInfo.searches.map((s:any)=>({num:s.searchNumber,query:s.searchQuery,results:s.resultCount,error:s.error})))}`);

        return{id:null,log:'x:409-nf',error:`409 duplicate but search failed to find match. Input="${n}"`,_409Debug:debugInfo};
      }
      console.log(`[v51] COMPANY CREATE FAILED: Status=${r.status}, err=${r.err}`);return{id:null,log:`x:${r.status}`,error:`HTTP ${r.status} - ${r.err}`};}
...
async function loadTitleMappings():Promise<void>{
  ...
  if(error){console.error('[v51] Error loading title mappings:',error.message);return;}
  ...
  titleMappingsLoadedAt=Date.now();console.log(`[v51] Loaded ${titleMappings.length} title mappings`);
}
...
async function loadAllJobTitles(k:string):Promise<void>{
  ...
  console.log(`[v51] Loaded ${allTitles.length} titles in ${Date.now()-startMs}ms (${apiCalls} calls)`);
}
...
Deno.serve(async(req)=>{
  console.log('[v51] Function invoked, method:', req.method, 'url:', req.url);
  ...
  try{
    ...
    console.log('[v51] action:', a, 'records:', recs?.length);
...
if(a==='refresh_title_cache'){titlesLoadedAt=0;allTitles=[];titleMappingsLoadedAt=0;titleMappings=[];Object.keys(JC).forEach(k=>delete JC[k]);await loadAllJobTitles(k);await loadTitleMappings();return R({success:true,v:'51',titlesLoaded:allTitles.length,loadedAt:new Date(titlesLoadedAt).toISOString(),titleMappingsLoaded:titleMappings.length,allTitles:allTitles.map(t=>({id:t.id,title:t.title}))});}
...
if(a==='push_companies'){for(const rec of recs){try{if(Date.now()-st>45000){results.push({id:rec.id,status:'error',message:'TO'});continue;}const cn=rec.company_name||rec.Name;if(!cn||bad(cn)){results.push({id:rec.id,status:'error',message:'M'});continue;}const r=await rC(cn,k);if(r.id){results.push({id:rec.id,name:cn,status:'success',crelateId:r.id,_409Debug:r._409Debug});await safeDbUpdate('marketing_companies',{crelate_id:r.id},'id',rec.id);}else results.push({id:rec.id,name:cn,status:'error',message:r.log,detail:r.error,_409Debug:r._409Debug});}catch(e){results.push({id:rec.id,status:'error',message:(e as Error).message});}}}
  ...
  console.log('[v51] push_jobs:', recs.length, 'records');
...
return R({success:true,v:'51',summary:{total:recs.length,ok:results.filter(r=>r.status==='success').length,skip:results.filter(r=>r.status==='skipped').length,err:results.filter(r=>r.status==='error').length},results,ms:`${Date.now()-st}`});
    ...
    console.error('[v51] TOP-LEVEL ERROR:', (topErr as Error).message, (topErr as Error).stack);

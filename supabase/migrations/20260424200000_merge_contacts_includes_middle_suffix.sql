-- Extend merge_contacts fill_cols to cover the newly added
-- middle_name and suffix columns. Same fill-if-empty semantics as the
-- other name/identity fields: canonical keeps its own value if set;
-- otherwise pick up the first non-empty value from any merged row.

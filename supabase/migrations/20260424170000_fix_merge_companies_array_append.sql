-- Replace merge_companies with a version that uses array_append()
-- explicitly instead of the `||` operator when adding fields to the
-- fields_filled accumulator. In plpgsql, `text[] || 'literal'` is
-- ambiguous: Postgres may resolve `||` to `text[] || text[]` and try
-- to cast the literal to an array, producing:
--   ERROR: malformed array literal: "last_searched_at"
-- which was hit the first time a merged row had a newer
-- last_searched_at than canonical. Switching to array_append keeps
-- intent explicit and avoids the ambiguity.
...
BEGIN
  ...
  IF EXISTS (SELECT 1 FROM information_schema.columns
    ...
    IF FOUND THEN fields_filled := array_append(fields_filled, 'is_high_priority'); END IF;
  END IF;
  ...
  IF EXISTS (SELECT 1 FROM information_schema.columns
    ...
    IF FOUND THEN fields_filled := array_append(fields_filled, 'has_md_cmo'); END IF;
  END IF;
  ...
  IF EXISTS (SELECT 1 FROM information_schema.columns
    ...
    IF FOUND THEN fields_filled := array_append(fields_filled, 'last_searched_at'); END IF;
  END IF;
